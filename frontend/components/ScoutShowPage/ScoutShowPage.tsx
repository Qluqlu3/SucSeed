// frontend/components/ScoutShowPage/ScoutShowPage.tsx
//
// match/scout_show テンプレートの React コンポーネント（スカウト一覧）。
// ScoutCheckPage と同構造（後継者へのスカウトを職人側が確認する画面）。
//
// 【data-props】
//   scouts: [{ pageId, name, birthday, avatarPath, matchTime, title }]

import { getCsrfToken } from '../../utils/csrf';
import { FlashMessages } from '../FlashMessages';

interface ScoutUser {
  pageId: string;
  name: string;
  birthday: string;
  avatarPath: string;
  matchTime: string;
  title: string;
}

interface Props {
  scouts: ScoutUser[];
  flash: Record<string, string>;
}

function calcAge(birthday: string): number {
  const fmt = (d: Date) => d.getFullYear() * 10000 + (d.getMonth() + 1) * 100 + d.getDate();
  return Math.floor((fmt(new Date()) - fmt(new Date(birthday))) / 10000);
}

function timeAgo(dateStr: string): string {
  const diffMs = Date.now() - new Date(dateStr).getTime();
  const minutes = Math.floor(diffMs / 60000);
  if (minutes < 60) return `${minutes}分`;
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return `${hours}時間`;
  const days = Math.floor(hours / 24);
  if (days < 30) return `${days}日`;
  const months = Math.floor(days / 30);
  if (months < 12) return `${months}ヶ月`;
  return `${Math.floor(months / 12)}年`;
}

export const ScoutShowPage = ({ scouts, flash }: Props) => (
  <div>
    <h1 className='mt-[2%] mb-[3%] pl-[2%] text-[71px] text-white bg-p-brand'>スカウト一覧</h1>

    <FlashMessages flash={flash} />
    <div className='min-h-screen'>
      <div className='w-[93%] mx-auto mb-[5%] p-[3%] bg-p-light border border-p-mid rounded-[7px]'>
        {scouts.length === 0 ? (
          <p className='mb-[60vh] text-[39px] text-p-dark'>まだありません</p>
        ) : (
          scouts.map((scout) => (
            <div key={scout.pageId} className='card mb-[1%]'>
              <div className='card-header bg-[#BAA9DA]'>
                {scout.matchTime}&nbsp;{timeAgo(scout.matchTime)}前
              </div>
              <a href={`/page/creator/${scout.pageId}`}>
                <div className='card-body bg-white'>
                  <div className='row'>
                    <div className='col-md-5 text-center'>
                      <img
                        src={scout.avatarPath}
                        className='img-circle'
                        width={230}
                        height={230}
                        alt={scout.name}
                      />
                    </div>
                    <div className='col-md-7 text-left'>
                      <p className='text-[61px] p-0 m-0'>{scout.name}</p>
                      <p className='text-[35px] p-0 my-[1%] ml-[71px]'>
                        {calcAge(scout.birthday)}
                        <small>歳</small>
                      </p>
                      <p className='text-[47px] p-0 m-0'>{scout.title}</p>
                    </div>
                  </div>
                </div>
              </a>
              <div className='card-footer pt-[0.2%] pb-0 px-0 m-0'>
                <div className='row'>
                  <div className='col-6 pr-[0.1%]'>
                    <form action={`/scout/ok/${scout.pageId}`} method='post'>
                      <input type='hidden' name='authenticity_token' value={getCsrfToken()} />
                      <button
                        type='submit'
                        className='btn btn-primary w-full m-0 py-[1%] text-[30px]'
                      >
                        話してみる
                      </button>
                    </form>
                  </div>
                  <div className='col-6 pl-[0.1%]'>
                    <form action={`/scout/sorry/${scout.pageId}`} method='post'>
                      <input type='hidden' name='authenticity_token' value={getCsrfToken()} />
                      <button
                        type='submit'
                        className='btn btn-danger w-full m-0 py-[1%] text-[30px]'
                      >
                        ごめんなさい
                      </button>
                    </form>
                  </div>
                </div>
              </div>
            </div>
          ))
        )}
      </div>
    </div>
  </div>
);
