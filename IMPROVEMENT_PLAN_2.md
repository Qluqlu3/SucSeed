...
- [ ] `user_controller.rb` — `email_certified` のバグを修正
...
- [ ] `match_controller.rb` — 6箇所のリダイレクト先を修正
...
- [ ] `my_page_controller.rb` — L4, L63 を `flash[:danger]` に修正
...
- [ ] `your_page_controller.rb` — `creator_show`・`heir_show` に nil ガードを追加
...
- [ ] 各ファイルで `find_by("id = ?", x)` → `find_by(id: x)` に変更
