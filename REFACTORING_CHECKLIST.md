...
#### ユーザー向けページ
- [ ] `EmailCertifiedPage` — flash Props + FlashMessages 挿入
- [ ] `user_controller.rb` — `email_certified` アクションに `flash: flash.to_h` 追加
- [ ] `CertifyPage` — flash Props + FlashMessages 挿入（certify コントローラ確認要）
- [ ] `CreatorShowPage` — flash Props + FlashMessages 挿入
- [ ] `CreatorUploadImagePage` — flash Props + FlashMessages 挿入
- [ ] `creator_controller.rb` — `show`, `upload_image` アクション確認・追加
- [ ] `ScoutShowPage` — コントローラ `scout_check` アクションの `@page_props` 実装含め確認要
- [ ] `Error404Page` / `Error500Page` — flash 不要か確認

#### 管理画面ページ（admin 系）
- [ ] `AdminCreatePage` — flash Props + FlashMessages 挿入
- [ ] `AdminLoginPage` — flash Props + FlashMessages 挿入
- [ ] `AdminDiaryCommentEditPage` — flash Props + FlashMessages 挿入
- [ ] `AdminDiaryEditPage` — flash Props + FlashMessages 挿入
- [ ] `AdminGalleryEditPage` — flash Props + FlashMessages 挿入
- [ ] `AdminIndexPage` — flash Props + FlashMessages 挿入
- [ ] `AdminInquiryEditPage` — flash Props + FlashMessages 挿入
- [ ] `AdminInquiryDetailPage` — flash Props + FlashMessages 挿入
- [ ] `AdminUserEditPage` — flash Props + FlashMessages 挿入
- [ ] `AdminSelectedUserEditPage` — flash Props + FlashMessages 挿入
- [ ] `admin_controller.rb` / `admin_edit_controller.rb` — 各アクションに `flash: flash.to_h` 追加
