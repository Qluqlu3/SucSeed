...
| 優先 | 内容 |
|---|---|
| 高 | `CertifyPage` 実装（現在プレースホルダー。ルート・コントローラ未整備） |
| 高 | `CreatorUploadImagePage` 実装（コントローラアクションがコメントアウト中） |
| 中 | `admin_edit#inquiry_detail` ルートが存在するがコントローラアクション未定義 → 削除 or 実装 |
| 低 | `get 'diary/show'` ルートが汎用すぎる → 不要なら削除 |
