...
| 優先 | 内容 |
|---|---|
| 中 | `CertifyPage` — プレースホルダー。ルート・コントローラ・実装が全て未整備。実装するか削除するか決定が必要 |
| 中 | `CreatorUploadImagePage` — プレースホルダー。`creator#data` / `creator#upload` / `creator#delete` がコメントアウト中。実装するか削除するか決定が必要 |
| 低 | `get 'your_page/your_page'` — controller/action 未指定ルート。`your_page#your_page` に自動マッピングされるが明示的に書いた方が良い |
