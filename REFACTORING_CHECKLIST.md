...
## Phase 10: ERB ビュー復元・render バグ修正 ✅

Phase 4 で一括削除した ERB ビューが実は全ページをクラッシュさせていたため再作成。

### ERB ビュー 47 ファイルを再作成（全て1行：`<div id="..." data-props="<%= @page_props.to_json %>"></div>`）
...
### コントローラの不正 render を修正（6箇所）

- [x] `user_controller.rb` — `password_reset` 成功時 `render :login` → `redirect_to "/index"` に変更
- [x] `user_controller.rb` — `password_reset` 失敗時 `render :password_edit` → `render :password_reset` に変更
- [x] `my_page_controller.rb` — `update` 失敗時 `render :show` → `render :update` に変更
- [x] `admin_edit_controller.rb` — `diary_delete`/`diary_comment_delete`/`gallery_delete` 失敗時の `render :diary` 等 → `redirect_to` に変更
- [x] `match_controller.rb` — `appeal_answer_ok` 失敗時 `render action: :appealed_list_view` → `redirect_to` に変更

### ApplicationController#render_404 実装

- [x] `application_controller.rb` — `render_404` アクション実装（`routes.rb` のワイルドカードルートで使用）
